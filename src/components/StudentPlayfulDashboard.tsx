...
const FloatingClouds = () => {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      <motion.div animate={{ x: ['-10%', '110%'] }} transition={{ repeat: Infinity, duration: 40, ease: 'linear' }} className="absolute top-10 left-0 text-white/50 text-8xl">☁️</motion.div>
      <motion.div animate={{ x: ['-10%', '110%'] }} transition={{ repeat: Infinity, duration: 30, ease: 'linear', delay: 10 }} className="absolute top-40 -left-20 text-white/40 text-7xl">☁️</motion.div>
      <motion.div animate={{ x: ['-10%', '110%'] }} transition={{ repeat: Infinity, duration: 45, ease: 'linear', delay: 5 }} className="absolute top-24 left-0 text-white/60 text-9xl">☁️</motion.div>
    </div>
  )
}
...
export default function StudentPlayfulDashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-sky-300 p-12 flex flex-col items-center justify-center">
      <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }} className="text-6xl mb-6">⏳</motion.div>
  ...
  const quests = [
    { id: 1, title: 'Read a new story', xp: 50, icon: '📖', done: true, color: 'bg-orange-100', shadow: 'shadow-[0_6px_0_#fed7aa]' },
    { id: 2, title: 'Math Ninja Quiz', xp: 30, icon: '🥷', done: false, color: 'bg-blue-100', shadow: 'shadow-[0_6px_0_#bfdbfe]' },
    { id: 3, title: 'Science Lab: Plants', xp: 40, icon: '🌱', done: false, color: 'bg-green-100', shadow: 'shadow-[0_6px_0_#bbf7d0]' },
  ]

  const badges = [
    { name: 'First Step', icon: '🐣', earned: true },
    { name: 'Math Star', icon: '⭐', earned: true },
    { name: 'Book Worm', icon: '🐛', earned: false },
    { name: 'Fire Streak', icon: '🔥', earned: false },
  ]

  const buttons = [
    { label: 'My Games', href: '/dashboard/parent/games', icon: '🎮', color: 'bg-pink-400', shadow: 'shadow-[0_8px_0_#db2777]' },
    { label: 'Videos', href: '/dashboard/parent/library', icon: '📺', color: 'bg-yellow-400', shadow: 'shadow-[0_8px_0_#ca8a04]' },
    { label: 'Rewards', href: '/dashboard/parent/rewards', icon: '🎁', color: 'bg-purple-400', shadow: 'shadow-[0_8px_0_#9333ea]' },
    { label: 'Homework', href: '/dashboard/parent/assignments', icon: '🎒', color: 'bg-cyan-400', shadow: 'shadow-[0_8px_0_#0891b2]' },
  ]
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1 w-full text-center md:text-left">
              <h1 className="text-4xl md:text-5xl font-black text-sky-900 mb-2 drop-shadow-sm">Hi, {firstName}! 👋</h1>
          ...
          <div className="lg:col-span-7">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-8 relative z-10">
                <h2 className="text-3xl sm:text-4xl font-black text-white drop-shadow-[0_4px_0_#047857]">🗺️ Quest Map</h2>
